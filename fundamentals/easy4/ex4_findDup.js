...
Algorithm 
- define a hash to keep track of the visited values
- iterate over the input array
  - if the number is a key in the hash, return the number (it's the duplibcate)
  - otherwise (it's not a key in the hash)
    - add it to the hash with a value of true
*/
...
function findDup(arr) {
  ...
  for (let i = 0; i < arr.length; i += 1) {
    if (visitedNums[arr[i]]) {
      return arr[i];
    } else { visitedNums[arr[i]] = true }
  }
}
