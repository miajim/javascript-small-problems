...
function minilang(str) {
  let commands = str.toLowerCase().split(' ');
  commands.forEach(command => Number.isNaN(Number(command)) ? TOKENS[command]() : register = Number(command));
}
...
Refactor the minilang function to include some error handling. In particular, the function should detect and report empty stack conditions,
and invalid tokens in the program. Ideally, the function should return an error message if an error occurs, 
or undefined if the program runs successfully.
